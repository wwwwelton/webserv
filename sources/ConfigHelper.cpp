...
ConfigHelper::DuplicatedValue::DuplicatedValue(const std::string& str)
    : LoadException(str) {
  _m = PARSE_ERROR "directive is duplicate \"" + str + "\"";
}

const char* ConfigHelper::DuplicatedValue::what(void) const throw() {
  return (_m.c_str());
}
...
int ConfigHelper::get_backlog(const std::vector<std::string>& tokens) {
  ...
  if (exists)
    throw DuplicatedValue(tokens[0]);
  if (tokens.size() != 2)
